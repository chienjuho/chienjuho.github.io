...
def main_cleveland():
    ...
    train_error = find_test_error(w, X_train_zscore, y_train)
    ...
    for reg in reg_lambdas:
        ...
        test_error = find_test_error(w, X_test_zscore, y_test)
        train_time = time.perf_counter() - tic
        print("L2 reg ", eta, reg, t, e_in[0], train_error, test_error, str(train_time) + " seconds",
    ...
    for reg in reg_lambdas:
        ...
        train_time = time.perf_counter() - tic
        # print(w)
        print("L1 reg ", eta, reg, t, e_in[0], train_error, test_error, str(train_time) + " seconds",
              "0vec=" + str(np.sum(np.abs(w) <= 10**(-5))))
...
def main_digits():
    ...
    train_error = find_test_error(w, X_train_zscore, y_train)
    test_error = find_test_error(w, X_test_zscore, y_test)
    print("baseline ", eta, t, e_in[0], train_error, test_error, str(train_time) + "seconds",
